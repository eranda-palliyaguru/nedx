...
$result1 = $db->prepare("SELECT * FROM sales WHERE customer_id='$customer_id' and action='active' ORDER by transaction_id DESC limit 0,1 ");
	...
	$result1 = $db->prepare("SELECT * FROM sms WHERE vehicle_no='$vehicle_no' ORDER by transaction_id DESC limit 0,1 ");
		...
		$smsday=$row1['date'];	
			      ...
			      $date = date("Y-m-d");
				  $sday1= strtotime( $smsday);
                  $nday1= strtotime($date);
                  $tdf1= abs($nday1-$sday1);
                  $nbday2= $tdf1/86400;
                  $rs2= intval($nbday2);
			
			echo $customer_id."_".$rs1."-".$rs2."<br>";
$set_day=90;			
if($set_day==0){
	$set_day=90;
}			
...
$user = "942018060603";
$password = "1669";
$text = urlencode("Hithawath Paribogikaya ".$row['vehicle_no']." Yathurupediye sewakalaya(service) lanvi ethi bevin eya apa sewasthanayata regena peminenna
HM Motors - 0777873343"
);
$to = $phone_number;
